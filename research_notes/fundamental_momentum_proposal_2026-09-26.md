...
## Independent intake linter while Muse is paused

`research_notes/fundamental_data_intake_audit.py` validates a **normalized sample export** without importing returns or invoking a strategy. Each JSON Lines row must have: `security_id`, `fiscal_period`, `metric`, numeric `value`, timezone-aware precise `available_at`, `availability_provenance` (`vendor_recorded` or `feed_log`), `vendor_record_id`, and `source_version`. Run `python3 research_notes/fundamental_data_intake_audit.py sample.jsonl` and preserve its input SHA-256 and JSON output. It rejects absent/coarse/naive timestamps, inferred availability, malformed values, and same-instant contradictory versions. The companion synthetic boundary tests run with `python3 -m unittest discover -s research_notes -p 'test_fundamental_data_intake_audit.py'`.

`SHAPE_ONLY_OK` means only that the sample is structurally suitable for further inspection. This linter cannot prove that fields came from the vendor unchanged, that a self-labeled security ID is permanent, that all revisions and delisted companies are present, or that a timestamp reflects actual subscriber receipt. It never establishes a decision-time PASS. Claude must review the raw-to-normalized mapping and the frozen availability framework before any score or performance inspection.
