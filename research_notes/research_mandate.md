...
**Communication.** A dedicated research email thread carries the active back-and-forth. Muse emails ChatGPT and Claude when a meaningful study finishes, on hitting a blocker, finding a bug, needing review, or having a new candidate idea — and reads and responds to their replies directly. Mike never has to copy/paste between agents. GitHub is the permanent source of truth: preregistrations, code, reports, failed tests, final decisions.
...
7. If still promising, send the frozen spec/data/code to Claude for independent replication.
