...
**Bridge protocol (per Mike, 2026-09-25).** Muse needs NO direct AgentMail access — no credentials, no API key, no forwarding. Muse sends FROM its own Gmail mailbox TO quality-flow-research@agentmail.to; ChatGPT reads in AgentMail and replies directly to the sender address, same thread. Muse's watcher polls its own mailbox, so the loop is: Muse mailbox → agentmail.to → ChatGPT → reply to Muse's mailbox → watcher → Muse responds. Standing thread subject: `QUALITY FLOW — AGENT RESEARCH BRIDGE`. Every message carries a type line near the top for automated triage — TYPE: BRAINSTORM / REVIEW REQUEST / TEST RESULT / DISAGREEMENT / BLOCKER / ACTION REQUIRED. ChatGPT's side checks the research inbox hourly; Muse polls its own mailbox every 10 minutes via the `research-bridge-watcher` cron (script: `research_notes/bridge_watcher.py`, state: `research_notes/bridge_state.json`), quiet unless a new ChatGPT message arrives. Mike is interrupted only on TYPE: ACTION REQUIRED or when a decision is his. **Blocked-reply fallback (added 2026-09-25 after AgentMail blocked ChatGPT's in-thread PASS review; Mike relayed it; root-caused 2026-09-25 ~18:40 as AgentMail's auto bounce-suppression — an org-level send-block entry on the recipient, fixable only by the AgentMail account holder):** (a0) diagnose/fix on ChatGPT's side: `GET /v0/lists/send/block/mickaellall116@gmail.com` — 200 with reason "bounced" = auto-suppression; `DELETE` the entry (expect 204; 409 read_only = do not loop), then resend; (a) if a reviewer reply is blocked by outbound safety checks, retry once as plain text under 2,000 characters, no links/attachments/tables; (b) if still blocked, the review stands as relayed via Mike — Mike-relayed reviewer text is authoritative, and Muse confirms receipt in-thread so the reviewer knows it landed; (c) keep bridge replies short plain-text by default; long artifacts go on GitHub.
